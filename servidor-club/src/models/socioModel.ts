...
class SocioModel {
	...
	async consultaHorario(idClase: string) {
		...
		const consulta: any = await this.db.query('SELECT C.ID_CLASE, group_concat( DC.Id_dias) AS "Dias", H.COMIENZO_HORARIO FROM CLASES C JOIN Horarios H ON C.Id_Horario = H.Id_Horario JOIN diasclases DC ON C.id_clase = DC.id_clase  WHERE C.Id_Clase = ? GROUP BY (C.Id_Clase);', [idClase]);
        //Ojo la consulta devuelve una tabla de una fila. (Array de array) Hay que desempaquetar y obtener la unica fila al enviar
		...
		async consultaHorario2(idsocio: string, horario: string) {
